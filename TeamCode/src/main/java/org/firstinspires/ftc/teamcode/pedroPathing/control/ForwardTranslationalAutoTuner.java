...
/**
 * Designs forward translational feedback after plant gains are set.
 * Reports unscaled plant {@code kV}/{@code kA} for reference; use Forward Plant
 * Identification for the {@code kS_x/kV_x/kA_x} values written into Constants.
 * Suggested {@code kP} goes on {@code forwardTranslationalController} (DoM D uses body vx).
 */
@TeleOp(name = "Forward Translational Auto Tuner", group = "3")
public class ForwardTranslationalAutoTuner extends OpMode {
    ...
    public void init_loop() {
        telemetry.addLine("Group 3: forward translational feedback (run after group 2 plant ID).");
        telemetry.addLine("Runs " + RUNTIME + " s at power " + POWER + ". Leave room.");
        telemetry.addLine("Put kP on foresightConfig.forwardTranslationalController.");
        telemetry.addLine("DoM D uses measured body-frame vx when kD is non-zero.");
        telemetry.update();
        follower.update();
    }
    ...
    public void loop() {
        ...
        if (!done) {
            ...
            double forwardVelocity = Math.abs(follower.velocity().toVector2D()
                    .dot(Vector2D.polar(1, follower.pose().heading())));
            vMax = Math.max(vMax, forwardVelocity / POWER);
            velocities.add(forwardVelocity);
            telemetry.addData("velocity (in/s)", String.format(Locale.US, "%.4f", forwardVelocity));
        ...
        telemetry.addData("plant kV_x (=1/K, unscaled)", String.format(Locale.US, "%.5f", plantKv));
        telemetry.addData("plant kA_x (=tau/K, unscaled)", String.format(Locale.US, "%.5f", plantKa));
        telemetry.addLine("--- Feedback for forwardTranslationalController ---");
    ...
    /**
     * Feedback kP design uses a detuned plant kA ({@code beta * tau/K}); plant gains
     * themselves must stay unscaled in Constants.
     */
    private double calculateKp(double beta) {
        double detunedKa = plantKa * beta;
        double denominator = foresightConfig.linearBrakeCoefficients.get().get(0, 0)
                + 2.0 * foresightConfig.quadraticBrakeCoefficients.get().get(0, 0) * vMax;
